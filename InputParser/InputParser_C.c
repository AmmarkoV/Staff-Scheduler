...
const char * _ipc_ver=" 0.34 ( 1/3 ) written from scratch - 8/2/10 \0";
...
/*
   ...
   Copies token with number (num) to c string (wheretostore) , variable storagesize contains the total size of wheretostore..!
*/
char * InputParser_GetWord_cstr(struct InputParserC * ipc,unsigned int num)
{
    if ( CheckWordNumOk(ipc,num) == 0 ) { return 0; }
    fprintf(stderr,"InputParser_GetWord_cstr returns non null terminated strings :P \n");
    return (char*) ipc->str[ipc->tokenlist[num].token_start];
}
